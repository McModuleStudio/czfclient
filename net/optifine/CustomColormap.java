...
public class CustomColormap implements CustomColors.IColorizer {
	...
	private void readColors() {
		try {
			...
			if (!flag || !flag1) {
				dbg("Non-standard palette size: " + i + "x" + j + ", should be: " + this.width + "x" + this.height
						+ ", path: " + s);
			}
	...
	public int getColor(BiomeGenBase p_getColor_1_, BlockPos p_getColor_2_) {
		return this.format == 0 ? this.getColorVanilla(p_getColor_1_, p_getColor_2_)
				: (this.format == 1 ? this.getColorGrid(p_getColor_1_, p_getColor_2_) : this.color);
	}

	public int getColorSmooth(IBlockAccess p_getColorSmooth_1_, double p_getColorSmooth_2_, double p_getColorSmooth_4_,
			double p_getColorSmooth_6_, int p_getColorSmooth_8_) {
		if (this.format == 2) {
			return this.color;
		} else {
	...
	private int getColorVanilla(BiomeGenBase p_getColorVanilla_1_, BlockPos p_getColorVanilla_2_) {
		double d0 = (double) MathHelper.clamp_float(p_getColorVanilla_1_.getFloatTemperature(p_getColorVanilla_2_),
				0.0F, 1.0F);
	...
	public void addMatchBlock(MatchBlock p_addMatchBlock_1_) {
		...
		this.matchBlocks = (MatchBlock[]) ((MatchBlock[]) Config.addObjectToArray(this.matchBlocks,
				p_addMatchBlock_1_));
	}
	...
	public String toString() {
		return "" + this.basePath + "/" + this.name + ", blocks: " + Config.arrayToString((Object[]) this.matchBlocks)
				+ ", source: " + this.source;
	}
}
