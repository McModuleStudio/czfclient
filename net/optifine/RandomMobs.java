...
public class RandomMobs {
		...
		if (working) {
			...
			try {
				...
				if (renderGlobal != null) {
					...
					ResourceLocation resourcelocation1 = randommobsproperties
							.getTextureLocation(p_getTextureLocation_0_, entityliving);
	...
	private static RandomMobsProperties parseProperties(ResourceLocation p_parseProperties_0_,
			ResourceLocation p_parseProperties_1_) {
		try {
			...
			if (inputstream == null) {
				...
				inputstream.close();
				RandomMobsProperties randommobsproperties = new RandomMobsProperties(properties, s,
						p_parseProperties_1_);
	...
	public static ResourceLocation getLocationIndexed(ResourceLocation p_getLocationIndexed_0_,
			int p_getLocationIndexed_1_) {
		if (p_getLocationIndexed_0_ == null) {
			...
			if (i < 0) {
				...
				ResourceLocation resourcelocation = new ResourceLocation(p_getLocationIndexed_0_.getResourceDomain(),
						s3);
	...
	private static ResourceLocation[] getTextureVariants(ResourceLocation p_getTextureVariants_0_) {
		...
		if (resourcelocation == null) {
			...
			if (list.size() <= 1) {
				...
				Config.dbg("RandomMobs: " + p_getTextureVariants_0_.getResourcePath() + ", variants: "
						+ aresourcelocation.length);
