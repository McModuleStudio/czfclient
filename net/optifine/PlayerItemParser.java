...
public class PlayerItemParser {
	...
	public static PlayerItemModel parseItemModel(JsonObject p_parseItemModel_0_) {
		...
		if (!Config.equals(s, "PlayerItem")) {
			...
			PlayerItemRenderer[] aplayeritemrenderer = (PlayerItemRenderer[]) ((PlayerItemRenderer[]) list
					.toArray(new PlayerItemRenderer[list.size()]));
	...
	private static PlayerItemRenderer parseItemRenderer(JsonObject p_parseItemRenderer_0_,
			Dimension p_parseItemRenderer_1_) {
	...
	private static ModelRenderer parseModelRenderer(JsonObject p_parseModelRenderer_0_,
			ModelBase p_parseModelRenderer_1_) {
		...
		if (jsonarray != null) {
			for (int j = 0; j < jsonarray.size(); ++j) {
				...
				modelrenderer.addBox(afloat2[0], afloat2[1], afloat2[2], (int) afloat2[3], (int) afloat2[4],
						(int) afloat2[5], f);
			}
		}
		...
		if (jsonarray1 != null) {
			for (int k = 0; k < jsonarray1.size(); ++k) {
				...
				modelrenderer.addSprite(afloat3[0], afloat3[1], afloat3[2], (int) afloat3[3], (int) afloat3[4],
						(int) afloat3[5], f1);
			}
		}
