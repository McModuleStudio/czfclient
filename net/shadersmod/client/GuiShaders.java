...
public class GuiShaders extends GuiScreen {
	...
	public void initGui() {
		...
		this.buttonList.add(new GuiButton(201, j1 / 4 - k1 / 2, this.height - 25, k1, j,
				Lang.get("of.options.shaders.shadersFolder")));
				...
				case 201:
					...
					try {
						...
						oclass.getMethod("browse", new Class[] { URI.class }).invoke(object,
								new Object[] { (new File(this.mc.mcDataDir, Shaders.shaderpacksdirname)).toURI() });
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		String s = "OpenGL: " + Shaders.glVersionString + ", " + Shaders.glVendorString + ", "
		...
		return s.contains("win") ? 1
				: (s.contains("mac") ? 2
						: (s.contains("solaris") ? 3
								: (s.contains("sunos") ? 3
										: (s.contains("linux") ? 4 : (s.contains("unix") ? 4 : 0)))));
	}
}
