...
public class ShadersTex {
	...
	public static void updateDynTexSubImage1(int[] src, int width, int height, int posX, int posY, int page) {
		...
		if (src.length >= j + i) {
			...
			GL11.glTexSubImage2D(GL11.GL_TEXTURE_2D, 0, posX, posY, width, height, GL12.GL_BGRA,
					GL12.GL_UNSIGNED_INT_8_8_8_8_REV, (IntBuffer) intbuffer);
			intbuffer.clear();
		}
	}
	...
	public static void allocateTextureMap(int texID, int mipmapLevels, int width, int height, Stitcher stitcher,
			TextureMap tex) {
	...
	public static void uploadTexSubForLoadAtlas(int[][] data, int width, int height, int xoffset, int yoffset,
			boolean linear, boolean clamp) {
	...
	public static int[][] readImageAndMipmaps(String name, int width, int height, int numLevels, boolean border,
			int defColor) {
	...
	public static void uploadTexSub1(int[][] src, int width, int height, int posX, int posY, int page) {
		...
		for (int k1 = posY; l > 0 && i1 > 0 && k < j; ++k) {
			...
			if (aint.length >= l1 * (page + 1)) {
				...
				GL11.glTexSubImage2D(GL11.GL_TEXTURE_2D, k, j1, k1, l, i1, GL12.GL_BGRA,
						GL12.GL_UNSIGNED_INT_8_8_8_8_REV, (IntBuffer) intbuffer);
			}
	...
	public static int blend4Alpha(int c0, int c1, int c2, int c3) {
		...
		int l1 = (k1 + 1) / 2;
		int i2 = j1 << 24
				| ((c0 >>> 16 & 255) * i + (c1 >>> 16 & 255) * j + (c2 >>> 16 & 255) * k + (c3 >>> 16 & 255) * l + l1)
						/ k1 << 16
				| ((c0 >>> 8 & 255) * i + (c1 >>> 8 & 255) * j + (c2 >>> 8 & 255) * k + (c3 >>> 8 & 255) * l + l1)
						/ k1 << 8
				| ((c0 >>> 0 & 255) * i + (c1 >>> 0 & 255) * j + (c2 >>> 0 & 255) * k + (c3 >>> 0 & 255) * l + l1)
						/ k1 << 0;
		return i2;
	}

	public static int blend4Simple(int c0, int c1, int c2, int c3) {
		int i = ((c0 >>> 24 & 255) + (c1 >>> 24 & 255) + (c2 >>> 24 & 255) + (c3 >>> 24 & 255) + 2) / 4 << 24
		...
		for (i = 0; w2 > 1 && h2 > 1; o2 = o1) {
			...
			for (int l1 = 0; l1 < h1; ++l1) {
				...
				for (int k2 = 0; k2 < w1; ++k2) {
					aint[i2 + k2] = blend4Alpha(aint[j2 + k2 * 2], aint[j2 + k2 * 2 + 1], aint[j2 + w2 + k2 * 2],
							aint[j2 + w2 + k2 * 2 + 1]);
				}
			}
	...
	public static void genMipmapSimple(int[] aint, int offset, int width, int height) {
		...
		for (i = 0; w2 > 1 && h2 > 1; o2 = o1) {
			...
			for (int l1 = 0; l1 < h1; ++l1) {
				...
				for (int k2 = 0; k2 < w1; ++k2) {
					aint[i2 + k2] = blend4Simple(aint[j2 + k2 * 2], aint[j2 + k2 * 2 + 1], aint[j2 + w2 + k2 * 2],
							aint[j2 + w2 + k2 * 2 + 1]);
				}
			}
	...
	public static void updateSubImage(MultiTexID multiTex, int[] src, int width, int height, int posX, int posY,
			boolean linear, boolean clamp) {
		int i = width * height;
		IntBuffer intbuffer = getIntBuffer(i);
		intbuffer.clear();
	...
	public static int blendColor(int color1, int color2, int factor1) {
		int i = 255 - factor1;
		return ((color1 >>> 24 & 255) * factor1 + (color2 >>> 24 & 255) * i) / 255 << 24
				| ((color1 >>> 16 & 255) * factor1 + (color2 >>> 16 & 255) * i) / 255 << 16
				| ((color1 >>> 8 & 255) * factor1 + (color2 >>> 8 & 255) * i) / 255 << 8
				| ((color1 >>> 0 & 255) * factor1 + (color2 >>> 0 & 255) * i) / 255 << 0;
	}
	...
	public static int[] loadAtlasSprite(BufferedImage bufferedimage, int startX, int startY, int w, int h, int[] aint,
