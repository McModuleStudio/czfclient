	...
	public void readStatFile() {
		if (this.statsFile.isFile()) {
			try {
				...
				logger.error((String) ("Couldn\'t parse statistics file " + this.statsFile),
					...
					if (((JsonElement) entry.getValue()).isJsonPrimitive()
						...
						if (jsonobject1.has("value") && jsonobject1.get("value").isJsonPrimitive()
							...
							tupleintjsonserializable
									.setIntegerValue(jsonobject1.getAsJsonPrimitive("value").getAsInt());
						}
		...
		for (Entry<StatBase, TupleIntJsonSerializable> entry : p_150880_0_.entrySet()) {
			if (((TupleIntJsonSerializable) entry.getValue()).getJsonSerializableValue() != null) {
				...
				jsonobject1.addProperty("value",
						(Number) Integer.valueOf(((TupleIntJsonSerializable) entry.getValue()).getIntegerValue()));
				...
				jsonobject.addProperty(((StatBase) entry.getKey()).statId,
						(Number) Integer.valueOf(((TupleIntJsonSerializable) entry.getValue()).getIntegerValue()));
			}
		}
