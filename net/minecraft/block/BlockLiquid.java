...
public abstract class BlockLiquid extends Block {
	...
	public boolean isBlockSolid(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
		...
		return material == this.blockMaterial ? false
				: (side == EnumFacing.UP ? true
						: (material == Material.ice ? false : super.isBlockSolid(worldIn, pos, side)));
	}

	public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
		return worldIn.getBlockState(pos).getBlock().getMaterial() == this.blockMaterial ? false
	...
	protected Vec3 getFlowVector(IBlockAccess worldIn, BlockPos pos) {
		...
		if (((Integer) worldIn.getBlockState(pos).getValue(LEVEL)).intValue() >= 8) {
			for (Object enumfacing1 : EnumFacing.Plane.HORIZONTAL) {
				...
				if (this.isBlockSolid(worldIn, blockpos1, (EnumFacing) enumfacing1)
						|| this.isBlockSolid(worldIn, blockpos1.up(), (EnumFacing) enumfacing1)) {
	...
	public static double getFlowDirection(IBlockAccess worldIn, BlockPos pos, Material materialIn) {
		...
		return vec3.xCoord == 0.0D && vec3.zCoord == 0.0D ? -1000.0D
				: MathHelper.func_181159_b(vec3.zCoord, vec3.xCoord) - (Math.PI / 2D);
	}
