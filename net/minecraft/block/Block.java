...
public class Block {
	...
	public int getMixedBrightnessForBlock(IBlockAccess worldIn, BlockPos pos) {
		...
		int i = worldIn.getCombinedLight(pos,
				Client.instance.getModuleManager().getModuleByClass(Xray.class).isEnabled() ? 100000
						: block.getLightValue());
	...
	public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
		if (Client.instance.getModuleManager().getModuleByClass(Xray.class).isEnabled()
				&& this == Block.getBlockById(16)) {
			return true;
		}
		if (Client.instance.getModuleManager().getModuleByClass(Xray.class).isEnabled()) {
			Xray xray = (Xray) Client.instance.getModuleManager().getModuleByClass(Xray.class);
			for (Integer block : xray.getBlocks()) {
				if (this != Block.getBlockById(block))
					continue;
				return true;
			}
		}
		return side == EnumFacing.DOWN && this.minY > 0.0 ? true
				: (side == EnumFacing.UP && this.maxY < 1.0 ? true
						: (side == EnumFacing.NORTH && this.minZ > 0.0 ? true
								: (side == EnumFacing.SOUTH && this.maxZ < 1.0 ? true
										: (side == EnumFacing.WEST && this.minX > 0.0 ? true
												: (side == EnumFacing.EAST && this.maxX < 1.0 ? true
	...
	public void addCollisionBoxesToList(World worldIn, BlockPos pos, IBlockState state, AxisAlignedBB mask,
		...
		if (collidingEntity == Helper.mc.thePlayer) {
			EventCollideWithBlock e = EventBus.getInstance().register(new EventCollideWithBlock(this, pos, axisalignedbb));
