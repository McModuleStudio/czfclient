	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		...
		if (i > 0) {
			...
			if (this.adjacentSourceBlocks >= 2 && this.blockMaterial == Material.water) {
				...
				if (iblockstate1.getBlock().getMaterial().isSolid()) {
					i1 = 0;
				} else if (iblockstate1.getBlock().getMaterial() == this.blockMaterial
						&& ((Integer) iblockstate1.getValue(LEVEL)).intValue() == 0) {
					i1 = 0;
				}
			}
		...
		if (this.canFlowInto(worldIn, pos.down(), iblockstate)) {
			if (this.blockMaterial == Material.lava
					&& worldIn.getBlockState(pos.down()).getBlock().getMaterial() == Material.water) {
		...
		for (Object enumfacing : EnumFacing.Plane.HORIZONTAL) {
			if (enumfacing != calculateFlowCost) {
				...
				if (!this.isBlocked(worldIn, blockpos, iblockstate)
					...
					if (distance < 4) {
						int j = this.func_176374_a(worldIn, blockpos, distance + 1,
