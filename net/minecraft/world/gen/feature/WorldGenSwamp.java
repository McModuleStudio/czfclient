...
public class WorldGenSwamp extends WorldGenAbstractTree {
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		...
		for (i = rand.nextInt(4) + 5; worldIn.getBlockState(position.down()).getBlock()
				.getMaterial() == Material.water; position = position.down()) {
			;
		}
			...
			for (int j = position.getY(); j <= position.getY() + 1 + i; ++j) {
				...
				for (int l = position.getX() - k; l <= position.getX() + k && flag; ++l) {
					for (int i1 = position.getZ() - k; i1 <= position.getZ() + k && flag; ++i1) {
						if (j >= 0 && j < 256) {
							Block block = worldIn.getBlockState(blockpos$mutableblockpos.func_181079_c(l, j, i1))
									.getBlock();
	...
	private void func_181647_a(World p_181647_1_, BlockPos p_181647_2_, PropertyBool p_181647_3_) {
		...
		for (p_181647_2_ = p_181647_2_
				.down(); p_181647_1_.getBlockState(p_181647_2_).getBlock().getMaterial() == Material.air
						&& i > 0; --i) {
			this.setBlockAndNotifyAdequately(p_181647_1_, p_181647_2_, iblockstate);
