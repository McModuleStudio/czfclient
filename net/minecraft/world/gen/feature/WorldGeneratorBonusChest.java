...
public class WorldGeneratorBonusChest extends WorldGenerator {
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		...
		while (((block = worldIn.getBlockState(position).getBlock()).getMaterial() == Material.air
				|| block.getMaterial() == Material.leaves) && position.getY() > 1) {
		...
		if (position.getY() < 1) {
			...
			for (int i = 0; i < 4; ++i) {
				BlockPos blockpos = position.add(rand.nextInt(4) - rand.nextInt(4), rand.nextInt(3) - rand.nextInt(3),
				...
				if (worldIn.isAirBlock(blockpos) && World.doesBlockHaveSolidTopSurface(worldIn, blockpos.down())) {
					...
					if (tileentity instanceof TileEntityChest) {
						WeightedRandomChestContent.generateChestContents(rand, this.chestItems,
					...
					if (worldIn.isAirBlock(blockpos2)
							&& World.doesBlockHaveSolidTopSurface(worldIn, blockpos2.down())) {
					...
					if (worldIn.isAirBlock(blockpos3)
							&& World.doesBlockHaveSolidTopSurface(worldIn, blockpos3.down())) {
						worldIn.setBlockState(blockpos3, Blocks.torch.getDefaultState(), 2);
					}

					if (worldIn.isAirBlock(blockpos4)
							&& World.doesBlockHaveSolidTopSurface(worldIn, blockpos4.down())) {
