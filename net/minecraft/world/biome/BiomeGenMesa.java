...
public class BiomeGenMesa extends BiomeGenBase {
	...
	public void genTerrainBlocks(World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int p_180622_4_,
		...
		if (this.field_150626_aH) {
			...
			double d0 = Math.min(Math.abs(p_180622_6_),
					this.field_150623_aE.func_151601_a((double) i * 0.25D, (double) j * 0.25D));
				...
				if (iblockstate1.getBlock().getMaterial() == Material.air) {
					...
					if (l == -1) {
						...
						if (i1 < l1 - 1) {
							...
							if (flag) {
								chunkPrimerIn.setBlockState(k1, i1, j1, Blocks.dirt.getDefaultState()
						...
						if (flag1) {
							chunkPrimerIn.setBlockState(k1, i1, j1, Blocks.stained_hardened_clay.getDefaultState()
									.withProperty(BlockColored.COLOR, EnumDyeColor.ORANGE));
						} else {
	...
	private IBlockState func_180629_a(int p_180629_1_, int p_180629_2_, int p_180629_3_) {
		int i = (int) Math.round(this.field_150625_aG.func_151601_a((double) p_180629_1_ * 1.0D / 512.0D,
				(double) p_180629_1_ * 1.0D / 512.0D) * 2.0D);
