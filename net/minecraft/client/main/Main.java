...
public class Main {
	public static void main(String[] p_main_0_) {
		...
		OptionSpec<Integer> optionspec1 = optionparser.accepts("port").withRequiredArg().ofType(Integer.class)
		...
		OptionSpec<Integer> optionspec6 = optionparser.accepts("proxyPort").withRequiredArg()
				.defaultsTo("8080", new String[0]).<Integer>ofType(Integer.class);
		...
		if (s != null) {
			try {
				proxy = new Proxy(Type.SOCKS,
						new InetSocketAddress(s, ((Integer) optionset.valueOf(optionspec6)).intValue()));
		...
		PropertyMap propertymap = (PropertyMap) gson.fromJson((String) optionset.valueOf(optionspec15),
				PropertyMap.class);
		PropertyMap propertymap1 = (PropertyMap) gson.fromJson((String) optionset.valueOf(optionspec16),
				PropertyMap.class);
		...
		File file3 = optionset.has(optionspec4) ? (File) optionset.valueOf(optionspec4)
				: new File(file1, "resourcepacks/");
		String s4 = optionset.has(optionspec10) ? (String) optionspec10.value(optionset)
		...
		GameConfiguration gameconfiguration = new GameConfiguration(
				...
				new GameConfiguration.ServerInformation(s6, integer.intValue()));
