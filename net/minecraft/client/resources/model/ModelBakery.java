...
public class ModelBakery {
				...
				for (IResource iresource : this.resourceManager.getAllResources(resourcelocation)) {
					...
					try {
						...
						throw new RuntimeException("Encountered an exception when loading model definition of \'"
								+ p_177586_1_ + "\' from: \'" + iresource.getResourceLocation()
			...
			for (ModelBlockDefinition.Variant modelblockdefinition$variant : ((ModelBlockDefinition.Variants) this.variants
				...
				if (this.models.get(resourcelocation) == null) {
					try {
						...
						LOGGER.warn((String) ("Unable to load block model: \'" + resourcelocation + "\' for variant: \'"
			...
			for (String s : this.getVariantNames(item)) {
				...
				if (this.models.get(resourcelocation) == null) {
					try {
						...
						LOGGER.warn((String) ("Unable to load item model: \'" + resourcelocation + "\' for item: \'"
								+ Item.itemRegistry.getNameForObject(item) + "\'"), (Throwable) exception);
					}
				}
			}
		}
	...
	private List<String> getVariantNames(Item p_177596_1_) {
		...
		if (list == null) {
			list = Collections.<String>singletonList(
