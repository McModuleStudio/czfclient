...
import gq.vapu.czfclient.Module.Modules.Render.ViewClip;
...
//	public void blurRender(int x, int y) {
////		if (OpenGlHelper.isFramebufferEnabled()) {
////			try {
////				this.theShaderGroup = new ShaderGroup(this.mc.getTextureManager(), this.resourceManager, this.mc.getFramebuffer(), new ResourceLocation("shaders/post/blur.json"));
////				this.theShaderGroup.createBindFramebuffers(x, y);
////				this.useShader = true;
////			} catch (IOException ioexception) {
////				logger.warn((String) ("Failed to load shader"), (Throwable) ioexception);
////				this.shaderIndex = shaderCount;
////				this.useShader = false;
////			} catch (JsonSyntaxException jsonsyntaxexception) {
////				logger.warn((String) ("Failed to load shader"), (Throwable) jsonsyntaxexception);
////				this.shaderIndex = shaderCount;
////				this.useShader = false;
////			}
////		}
////	}
	//This Render void Will Made U low fps lol
	...
	public void getMouseOver(float partialTicks) {
		...
		if (entity != null && this.mc.theWorld != null) {
			...
			List list = this.mc.theWorld.getEntitiesInAABBexcluding(entity,
					...
					Predicates.and(EntitySelectors.NOT_SPECTATING, new EntityRenderer$1(this)));
			double d2 = d1;
		...
		if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
			if (!this.mc.gameSettings.debugCamEnable) {
				...
				if (entity instanceof EntityAnimal) {
					...
					f6 = entityanimal.prevRotationYawHead
				...
				Object object = Reflector.newInstance(Reflector.EntityViewRenderEvent_CameraSetup_Constructor,
						new Object[] { this, entity, block1, Float.valueOf(partialTicks), Float.valueOf(f6),
								Float.valueOf(f7), Float.valueOf(f8) });
			...
			if (entity instanceof EntityAnimal) {
				...
				GlStateManager.rotate(entityanimal1.prevRotationYawHead
						+ (entityanimal1.rotationYawHead - entityanimal1.prevRotationYawHead) * partialTicks + 180.0F,
						0.0F, 1.0F, 0.0F);
			} else {
				GlStateManager.rotate(
						entity.prevRotationYaw + (entity.rotationYaw - entity.prevRotationYaw) * partialTicks + 180.0F,
	...
	public void renderHand(float p_renderHand_1_, int p_renderHand_2_, boolean p_renderHand_3_, boolean p_renderHand_4_,
		...
		if (!this.debugView) {
			...
			Project.gluPerspective(this.getFOVModifier(p_renderHand_1_, false),
			...
			if (p_renderHand_3_) {
				...
				flag = this.mc.getRenderViewEntity() instanceof EntityLivingBase
						&& ((EntityLivingBase) this.mc.getRenderViewEntity()).isPlayerSleeping();
				boolean flag1 = !ReflectorForge.renderFirstPersonHand(this.mc.renderGlobal, p_renderHand_1_,
						p_renderHand_2_);
	...
	private void updateLightmap(float partialTicks) {
		if (this.lightmapUpdateNeeded) {
			...
			if (worldclient != null) {
				if (Config.isCustomColors() && CustomColors.updateLightmap(worldclient, this.torchFlickerX,
						this.lightmapColors, this.mc.thePlayer.isPotionActive(Potion.nightVision))) {
	...
	private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
		...
		if (flag) {
			ShadersRender.setupTerrain(renderglobal, entity, (double) partialTicks, frustum, this.frameCount++,
	...
	private void updateFogColor(float partialTicks) {
		...
		if (this.cloudFog) {
			...
			Vec3 vec34 = CustomColors.getUnderwaterColor(this.mc.theWorld, this.mc.getRenderViewEntity().posX,
	...
	private void setupFog(int p_78468_1_, float partialTicks) {
		...
		if (Reflector.ForgeHooksClient_getFogDensity.exists()) {
			f1 = Reflector.callFloat(Reflector.ForgeHooksClient_getFogDensity,
		...
		if (world != null) {
			if (Config.getNewRelease() != null) {
				...
				ChatComponentText chatcomponenttext = new ChatComponentText(
						I18n.format("of.message.newVersion", new Object[] { s1 }));
		...
		if (this.mc.theWorld != null) {
			...
			if (i > this.lastErrorCheckTimeMs + 10000L) {
				...
				if (j != 0) {
					...
					ChatComponentText chatcomponenttext = new ChatComponentText(
							I18n.format("of.message.openglError", new Object[] { Integer.valueOf(j), s }));
					this.mc.ingameGUI.getChatGUI().printChatMessage(chatcomponenttext);
				}
			}
		}
	...
	public boolean setFxaaShader(int p_setFxaaShader_1_) {
		if (!OpenGlHelper.isFramebufferEnabled()) {
			return false;
		} else if (this.theShaderGroup != null && this.theShaderGroup != this.fxaaShaders[2]
