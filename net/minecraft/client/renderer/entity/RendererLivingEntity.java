...
import gq.vapu.czfclient.Module.Modules.Render.ESP;
import gq.vapu.czfclient.Module.Modules.Render.NameTag;
import gq.vapu.czfclient.Util.Render.OutlineUtils;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
	...
	public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
		if (!Reflector.RenderLivingEvent_Pre_Constructor.exists()
			...
			try {
				...
				float f8;
				if(entity == Minecraft.getMinecraft().thePlayer && Scaffold.grabBlockSlot() != -1 &&
						Client.instance.getModuleManager().getModuleByClass(Scaffold.class).isEnabled()&&Scaffold.fucku==1) {
					f8=Scaffold.pitch;
				}else if(entity == Minecraft.getMinecraft().thePlayer && Killaura.curTarget != null
						&& Client.instance.getModuleManager().getModuleByClass(Killaura.class).isEnabled()) {
					f8=Killaura.facing[1];
				}else {
	...
	public void renderName(T entity, double x, double y, double z) {
		if (Client.instance.getModuleManager().getModuleByClass(NameTag.class).isEnabled()
