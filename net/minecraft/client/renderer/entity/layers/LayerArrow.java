...
public class LayerArrow implements LayerRenderer<EntityLivingBase> {
	...
	public void doRenderLayer(EntityLivingBase entitylivingbaseIn, float p_177141_2_, float p_177141_3_,
		...
		if (i > 0) {
			Entity entity = new EntityArrow(entitylivingbaseIn.worldObj, entitylivingbaseIn.posX,
			...
			for (int j = 0; j < i; ++j) {
				...
				entity.prevRotationYaw = entity.rotationYaw = (float) (Math.atan2((double) f, (double) f2) * 180.0D
						/ Math.PI);
				entity.prevRotationPitch = entity.rotationPitch = (float) (Math.atan2((double) f1, (double) f6) * 180.0D
						/ Math.PI);
				double d0 = 0.0D;
				double d1 = 0.0D;
				double d2 = 0.0D;
