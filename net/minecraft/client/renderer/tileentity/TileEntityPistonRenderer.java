...
public class TileEntityPistonRenderer extends TileEntitySpecialRenderer<TileEntityPiston> {
	...
	public void renderTileEntityAt(TileEntityPiston te, double x, double y, double z, float partialTicks,
		...
		if (block.getMaterial() != Material.air && te.getProgress(partialTicks) < 1.0F) {
			...
			worldrenderer.setTranslation((double) ((float) x - (float) blockpos.getX() + te.getOffsetX(partialTicks)),
					(double) ((float) y - (float) blockpos.getY() + te.getOffsetY(partialTicks)),
			...
			if (block == Blocks.piston_head && te.getProgress(partialTicks) < 0.5F) {
				...
				worldrenderer.setTranslation((double) ((float) x - (float) blockpos.getX()),
						(double) ((float) y - (float) blockpos.getY()), (double) ((float) z - (float) blockpos.getZ()));
