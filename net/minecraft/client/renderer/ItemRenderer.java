...
import gq.vapu.czfclient.Module.Modules.Render.Animations;
...
public class ItemRenderer
{
    ...
    private void func_178105_d(float p_178105_1_)
    {
        ...
        if(Client.getModuleManager().getModuleByName("Animations").isEnabled()&&Animations.smooth.getValue().booleanValue()) {
        GlStateManager.translate(f, f1+0.15, f2+0.05);
    }else {
        GlStateManager.translate(f, f1, f2);
    }
        }
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        ...
        if (this.itemToRender != null)
        {
            ...
            {
            	if(Client.instance.getModuleManager().getModuleByName("EveryThingBlock").isEnabled()&&mc.gameSettings.keyBindUseItem.isKeyDown()) {
            		if(Client.instance.getModuleManager().getModuleByName("Animations").isEnabled()) {
                		if (Animations.mode.getValue()==Animations.renderMode.Sigma) {
                            this.transformFirstPersonItem(f, 0.0f);
                            this.func_178103_d();
