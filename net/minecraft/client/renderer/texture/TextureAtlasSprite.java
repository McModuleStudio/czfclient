...
public class TextureAtlasSprite {
	...
	protected static TextureAtlasSprite makeAtlasSprite(ResourceLocation spriteResourceLocation) {
		...
		return (TextureAtlasSprite) (locationNameClock.equals(s) ? new TextureClock(s)
		...
		if (this.animationMetadata != null) {
			...
			if (this.tickCounter >= this.animationMetadata.getFrameTimeSingle(this.frameCounter)) {
				...
				if (i != k && k >= 0 && k < this.framesTextureData.size()) {
					TextureUtil.uploadTextureMipmap((int[][]) ((int[][]) this.framesTextureData.get(k)), this.width,
	...
	private void updateAnimationInterpolated() {
		...
		if (i != k && k >= 0 && k < this.framesTextureData.size()) {
			...
			for (int l = 0; l < aint.length; ++l) {
				...
				if (l < aint1.length && aint1[l].length == aint[l].length) {
					for (int i1 = 0; i1 < aint[l].length; ++i1) {
						int j1 = aint[l][i1];
						int k1 = aint1[l][i1];
						int l1 = (int) ((double) ((j1 & 16711680) >> 16) * d0
								+ (double) ((k1 & 16711680) >> 16) * (1.0D - d0));
			...
			TextureUtil.uploadTextureMipmap(this.interpolatedFrameData, this.width, this.height, this.originX,
					this.originY, false, false);
		}
	}
	...
	public void loadShaderSpriteFrames(ResourceLocation p_loadShaderSpriteFrames_1_, int p_loadShaderSpriteFrames_2_)
			throws IOException {
		...
		if (animationmetadatasection == null) {
			...
			if (animationmetadatasection.getFrameCount() > 0) {
				...
				this.animationMetadata = new AnimationMetadataSection(list, this.width, this.height,
						animationmetadatasection.getFrameTime(), animationmetadatasection.isInterpolate());
			}
		}
	}
}
