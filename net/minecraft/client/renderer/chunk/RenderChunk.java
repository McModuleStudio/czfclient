...
public class RenderChunk {
	...
	public void rebuildChunk(float x, float y, float z, ChunkCompileTaskGenerator generator) {
		...
		try {
			...
			regionrendercache = this.createRegionRenderCache(this.world, blockpos.add(-1, -1, -1),
					blockpos1.add(1, 1, 1), 1);

			if (Reflector.MinecraftForgeClient_onRebuildChunk.exists()) {
				Reflector.call(Reflector.MinecraftForgeClient_onRebuildChunk,
						new Object[] { this.world, this.position, regionrendercache });
			}
		...
		if (!regionrendercache.extendedLevelsInChunkCache()) {
			...
			while (iterator.hasNext()) {
				...
				for (int i = 0; i < aenumworldblocklayer.length; ++i) {
					...
					if (flag2) {
						boolean flag4 = Reflector.callBoolean(block, Reflector.ForgeBlock_canRenderInLayer,
					...
					if (block.getRenderType() != -1) {
						...
						aboolean[j] |= blockrendererdispatcher.renderBlock(iblockstate, blockposm, regionrendercache,
								worldrenderer);
					}
				}
			}
			...
			for (EnumWorldBlockLayer enumworldblocklayer1 : ENUM_WORLD_BLOCK_LAYERS) {
				...
				if (compiledchunk.isLayerStarted(enumworldblocklayer1)) {
					...
					this.postRenderBlocks(enumworldblocklayer1, x, y, z,
							generator.getRegionRenderCacheBuilder().getWorldRendererByLayer(enumworldblocklayer1),
							compiledchunk);
				}
			}
	...
	protected RegionRenderCache createRegionRenderCache(World p_createRegionRenderCache_1_,
			BlockPos p_createRegionRenderCache_2_, BlockPos p_createRegionRenderCache_3_,
			int p_createRegionRenderCache_4_) {
		return new RegionRenderCache(p_createRegionRenderCache_1_, p_createRegionRenderCache_2_,
				p_createRegionRenderCache_3_, p_createRegionRenderCache_4_);
	}
