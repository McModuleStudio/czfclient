...
import gq.vapu.czfclient.API.Events.Render.EventRenderBlock;
import gq.vapu.czfclient.API.eventapi.EventManager;
...
public class BlockRendererDispatcher implements IResourceManagerReloadListener {
	...
	public void renderBlockDamage(IBlockState state, BlockPos pos, TextureAtlasSprite texture,
		...
		if (i == 3) {
			...
			if (Reflector.ISmartBlockModel.isInstance(ibakedmodel)) {
				IBlockState iblockstate = (IBlockState) Reflector.call(block, Reflector.ForgeBlock_getExtendedState,
						new Object[] { state, blockAccess, pos });

				for (EnumWorldBlockLayer enumworldblocklayer : EnumWorldBlockLayer.values()) {
					if (Reflector.callBoolean(block, Reflector.ForgeBlock_canRenderInLayer,
							new Object[] { enumworldblocklayer })) {
						Reflector.callVoid(Reflector.ForgeHooksClient_setRenderLayer,
								new Object[] { enumworldblocklayer });
						IBakedModel ibakedmodel2 = (IBakedModel) Reflector.call(ibakedmodel,
								Reflector.ISmartBlockModel_handleBlockState, new Object[] { iblockstate });
						IBakedModel ibakedmodel3 = (new SimpleBakedModel.Builder(ibakedmodel2, texture))
								.makeBakedModel();
						this.blockModelRenderer.renderModel(blockAccess, ibakedmodel3, state, pos,
	...
	public boolean renderBlock(IBlockState state, BlockPos pos, IBlockAccess blockAccess,
		...
		/*
		 * final EventRenderBlock event = new EventRenderBlock(pos.getX(), pos.getY(),
		 * pos.getZ(), state.getBlock()); EventBus.getInstance().register(event);
		 */
		try {
			...
			if (i == -1) {
				...
				case 3:
					...
					boolean flag = this.blockModelRenderer.renderModel(blockAccess, ibakedmodel, state, pos,
							worldRendererIn);
	...
	public IBakedModel getModelFromBlockState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		...
		if (Reflector.ISmartBlockModel.isInstance(ibakedmodel)) {
			IBlockState iblockstate = (IBlockState) Reflector.call(block, Reflector.ForgeBlock_getExtendedState,
					new Object[] { state, worldIn, pos });
			ibakedmodel = (IBakedModel) Reflector.call(ibakedmodel, Reflector.ISmartBlockModel_handleBlockState,
