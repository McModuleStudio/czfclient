...
import gq.vapu.czfclient.Module.Modules.Render.HUD;
...
public class GuiIngame extends Gui {
	...
	public void renderGameOverlay(float partialTicks) {
		...
		if (this.recordPlayingUpFor > 0) {
			...
			if (k1 > 8) {
				...
				GlStateManager.translate((float) (i / 2), (float) (j - 28), 0.0F);
				GlStateManager.enableBlend();
				GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
				int i1 = 16777215;
		...
		CFontRenderer font = FontLoaders.Comfortaa20;
		CFontRenderer font1 = FontLoaders.Comfortaa20;
		...
		EventBus.getInstance().register(new EventRender2D(partialTicks));
	...
	protected void renderTooltip(ScaledResolution sr, float partialTicks) {
		if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
			...
			this.zLevel = -90.0F;
			// 选中物品栏线条
			RenderUtil.drawRect(i - 91 + entityplayer.inventory.currentItem * 20,
					this.mc.ingameGUI.getChatGUI().getChatOpen() ? sr.getScaledHeight() + 50 : sr.getScaledHeight() - 27,
					i - 69 + entityplayer.inventory.currentItem * 20,
					this.mc.ingameGUI.getChatGUI().getChatOpen() ? sr.getScaledHeight() : sr.getScaledHeight(),
					new Color(180, 61, 236, 255).getRGB());

			// 渲染屏幕下方的PING值之类的信息
			if (Client.instance.getModuleManager().getModuleByClass(HUD.class).isEnabled() && !this.mc.ingameGUI.getChatGUI().getChatOpen()) {
				CFontRenderer font = FontLoaders.Comfortaa17;
				// 信息下阴影
				//Gui.drawRect(0, sr.getScaledHeight() - 22, RenderUtil.width(), sr.getScaledHeight(), new Color(28, 28, 28, 200).getRGB());
				BlurUtil.blurAreaBoarder(0, sr.getScaledHeight() - 22, RenderUtil.width(), sr.getScaledHeight(),150);
				if (!mc.isSingleplayer()) {
					//int PING = mc.getNetHandler().getPlayerInfo(mc.thePlayer.getUniqueID()).getResponseTime();
					int PING = HUD.ping;
					Color Worst = null;
					if (PING <= 100) {
						Worst = new Color(0xAE00FF);
					} else if (PING > 100 && PING <= 250) {
						Worst = Color.ORANGE;
					} else if (PING > 250) {
						Worst = Color.ORANGE;
					}
					this.drawFilledCircle(10, sr.getScaledHeight() - 10, 3, Worst);
					FontLoaders.Comfortaa17.drawString("PING:" + PING + "     FPS:" + mc.debugFPS, 16, sr.getScaledHeight() - 11.5f, -1);
				} else {
					this.drawFilledCircle(10, mc.displayHeight / 2 - 10, 3, new Color(0x73B46B));
					FontLoaders.Comfortaa17.drawString(/*DOG NAN*/"PING:NaN     FPS:" + mc.debugFPS, 16, sr.getScaledHeight() - 11.5f, -1);
					//向魔怔のsfg致敬！
					//sfg..嘿嘿..我的sfg
				}
				//String fuck="SkidBlue Version "+Client.ver+" - "+ Minecraft.getMinecraft().thePlayer.getName();
				String fuck = mc.isIntegratedServerRunning() ? "Integrated Server" : mc.getCurrentServerData().serverIP;
				FontLoaders.Comfortaa17.drawString(fuck, sr.getScaledWidth() - FontLoaders.Comfortaa17.getStringWidth(fuck + " "), sr.getScaledHeight() - 11.5f, -1);
			}
	...
	public void renderHorseJumpBar(ScaledResolution p_175186_1_, int p_175186_2_) {
		...
		int j = this.mc.ingameGUI.getChatGUI().getChatOpen() ? p_175186_1_.getScaledHeight() - 42
				: p_175186_1_.getScaledHeight() - 32 + 3;
	...
	public void renderExpBar(ScaledResolution p_175176_1_, int p_175176_2_) {
		...
		if (i > 0) {
			...
			int j = this.mc.ingameGUI.getChatGUI().getChatOpen() ? p_175176_1_.getScaledHeight() - 42
					: p_175176_1_.getScaledHeight() - 29;
		...
		if (this.mc.thePlayer.experienceLevel > 0) {
			...
			int l = this.mc.ingameGUI.getChatGUI().getChatOpen() ? p_175176_1_.getScaledHeight() - 47
					: p_175176_1_.getScaledHeight() - 35;
			boolean flag = false;
	...
	public void func_181551_a(ScaledResolution p_181551_1_) {
		...
		if (this.remainingHighlightTicks > 0 && this.highlightingItemStack != null) {
			...
			int j = this.mc.ingameGUI.getChatGUI().getChatOpen() ? p_181551_1_.getScaledHeight() - 76
					: p_181551_1_.getScaledHeight() - 62;
	...
	private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_) {
		...
		String s;
		for (Iterator var9 = arraylist1.iterator(); var9
				.hasNext(); i = Math.max(i, this.mc.fontRendererObj.getStringWidth(s))) {
			Object score0 = var9.next();
			Score score = (Score) score0;
			ScorePlayerTeam scoreplayerteam = scoreboard.getPlayersTeam(score.getPlayerName());
			s = ScorePlayerTeam.formatPlayerName(scoreplayerteam, score.getPlayerName()) + ": " + EnumChatFormatting.RED
					+ score.getScorePoints();
		}

		int j1 = arraylist1.size() * this.mc.fontRendererObj.FONT_HEIGHT;
		int k1 = (int) ((double) p_180475_2_.getScaledHeight() - new ScaledResolution(this.mc).getScaledHeight() / 8);
		byte b0 = 3;
		int j = p_180475_2_.getScaledWidth() - i - b0;
		int k = 0;
		Iterator var14 = arraylist1.iterator();

		while (var14.hasNext()) {
			Object score10 = var14.next();
			Score score1 = (Score) score10;
			++k;
			ScorePlayerTeam scoreplayerteam1 = scoreboard.getPlayersTeam(score1.getPlayerName());
			String s1 = ScorePlayerTeam.formatPlayerName(scoreplayerteam1, score1.getPlayerName());
			String s2 = "" + EnumChatFormatting.RED + score1.getScorePoints();
			int l = k1 - k * this.mc.fontRendererObj.FONT_HEIGHT;
			int i1 = p_180475_2_.getScaledWidth() - b0 + 2;
			drawRect(j - 2, l, i1 + 2, l + this.mc.fontRendererObj.FONT_HEIGHT, 1342177280);
			//this.mc.entityRenderer.blurRender(j - 2, l + this.mc.fontRendererObj.FONT_HEIGHT); //low fpser
			this.mc.fontRendererObj.drawString(s1, (int) j, (int) l, -1);
			this.mc.fontRendererObj.drawString(s2, (int) (i1 - this.mc.fontRendererObj.getStringWidth(s2)), (int) l,
					-1);
			if (k == arraylist1.size()) {
				String s3 = p_180475_1_.getDisplayName();
				drawRect(j - 2, l - this.mc.fontRendererObj.FONT_HEIGHT - 1, i1 + 2, l - 1, 1610612736);
				drawRect(j - 2, l - 1, i1 + 2, l, 1342177280);
				this.mc.fontRendererObj.drawString(s3,
						(int) (j + i / 2 - this.mc.fontRendererObj.getStringWidth(s3) / 2),
						(int) (l - this.mc.fontRendererObj.FONT_HEIGHT), -1);
			}
		}
	}
	...
	private void renderBossHealth() {
		if (BossStatus.bossName != null && BossStatus.statusBarTime > 0) {
			...
			this.getFontRenderer().drawStringWithShadow(s,
					(float) (i / 2 - this.getFontRenderer().getStringWidth(s) / 2), (float) (b0 - 10), l);
			GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
			this.mc.getTextureManager().bindTexture(icons);
		}
	}
