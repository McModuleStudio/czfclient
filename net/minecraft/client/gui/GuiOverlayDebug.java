...
public class GuiOverlayDebug extends Gui {
	...
	protected List call() {
		...
		if (this.isReducedDebug()) {
			...
			if (this.mc.theWorld != null && this.mc.theWorld.isBlockLoaded(blockpos)) {
				...
				if (this.mc.isIntegratedServerRunning() && this.mc.getIntegratedServer() != null) {
					...
					if (entityplayermp != null) {
						difficultyinstance = entityplayermp.worldObj
								.getDifficultyForLocation(new BlockPos(entityplayermp));
					}
				}

				arraylist.add(String.format("Local Difficulty: %.2f (Day %d)",
			...
			if (this.mc.objectMouseOver != null
				...
				arraylist.add(String.format("Looking at: %d %d %d", new Object[] { Integer.valueOf(blockpos1.getX()),
	...
	protected List getDebugInfoRight() {
		...
		if (Reflector.FMLCommonHandler_getBrandings.exists()) {
			...
			arraylist.add("");
			arraylist.addAll((Collection) Reflector.call(object, Reflector.FMLCommonHandler_getBrandings,
		...
		if (this.isReducedDebug()) {
			...
			if (this.mc.objectMouseOver != null
				...
				for (Iterator iterator = iblockstate.getProperties().entrySet().iterator(); ((Iterator) iterator)
						.hasNext(); arraylist.add(((IProperty) entry.getKey()).getName() + ": " + s)) {
	...
	public static final class GuiOverlayDebug$1 {
		public static final int[] field_178907_a = new int[EnumFacing.values().length];
