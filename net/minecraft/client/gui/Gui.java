...
public class Gui {
	...
	public static void drawRainbowRectVertical(int x, int y, int x1, int y1, int segments, float alpha) {
		...
		for (int i = 0; i < segments; ++i) {
			Minecraft.getMinecraft().ingameGUI.drawGradientRect(x, y + segmentLength * i - 1, x1,
					y + (segmentLength + 1) * i, Gui.rainbow(time, (float) i * 0.1f, alpha).getRGB(),
					Gui.rainbow(time, ((float) i + 0.1f) * 0.1f, alpha).getRGB());
		}
	}
	...
	public static Color rainbow(long time, float count, float fade) {
		...
		long color = Long.parseLong(
				(String) Integer.toHexString((int) Color.HSBtoRGB((float) hue, (float) 1.0f, (float) 1.0f)), (int) 16);
