...
public class GuiWinGame extends GuiScreen {
		...
		if (this.field_146582_i == null) {
			...
			try {
				...
				while ((s = bufferedreader.readLine()) != null) {
					...
					for (s = s.replaceAll("PLAYERNAME", this.mc.getSession().getUsername()); s
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
			...
			if ((float) l + f + 12.0F + 8.0F > 0.0F && (float) l + f < (float) this.height) {
				...
				if (s.startsWith("[C]")) {
					this.fontRendererObj.drawStringWithShadow(s.substring(3),
							(float) (j + (i - this.fontRendererObj.getStringWidth(s.substring(3))) / 2), (float) l,
							16777215);
				} else {
		...
		int k1 = this.height;
		worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
		worldrenderer.pos(0.0D, (double) k1, (double) this.zLevel).tex(0.0D, 1.0D).color(1.0F, 1.0F, 1.0F, 1.0F)
				.endVertex();
		worldrenderer.pos((double) j1, (double) k1, (double) this.zLevel).tex(1.0D, 1.0D).color(1.0F, 1.0F, 1.0F, 1.0F)
				.endVertex();
		worldrenderer.pos((double) j1, 0.0D, (double) this.zLevel).tex(1.0D, 0.0D).color(1.0F, 1.0F, 1.0F, 1.0F)
				.endVertex();
