...
public class SoundManager {
	...
	public SoundManager(SoundHandler p_i45119_1_, GameSettings p_i45119_2_) {
		...
		try {
			...
			logger.error(LOG_MARKER, (String) "Error linking with the LibraryJavaSound plug-in",
					(Throwable) soundsystemexception);
		}
	}
			...
			try {
				(new Thread(new Runnable() {
					public void run() {
						...
						SoundManager.this.sndSystem
								.setMasterVolume(SoundManager.this.options.getSoundLevel(SoundCategory.MASTER));
						SoundManager.logger.info(SoundManager.LOG_MARKER, "Sound engine started");
					}
				}, "Sound Library Loader")).start();
			} catch (RuntimeException runtimeexception) {
				logger.error(LOG_MARKER, (String) "Error starting SoundSystem. Turning off sounds & music",
			...
			if (category == SoundCategory.MASTER) {
				...
				for (String s : this.categorySounds.get(category)) {
					...
					float f = this.getNormalizedVolume(isound,
							(SoundPoolEntry) this.playingSoundPoolEntries.get(isound), category);
	...
	public boolean isSoundPlaying(ISound sound) {
		if (!this.loaded) {
			...
			return s == null ? false
					: this.sndSystem.playing(s) || this.playingSoundsStopTime.containsKey(s)
					...
					if (soundpoolentry == SoundHandler.missing_sound) {
						...
						if (f2 == 0.0F) {
							...
							if (soundpoolentry.isStreamingSound()) {
								this.sndSystem.newStreamingSource(false, s, getURLForSoundResource(resourcelocation),
										resourcelocation.toString(), flag, sound.getXPosF(), sound.getYPosF(),
										sound.getZPosF(), sound.getAttenuationType().getTypeInt(), f1);
							} else {
								this.sndSystem.newSource(false, s, getURLForSoundResource(resourcelocation),
										resourcelocation.toString(), flag, sound.getXPosF(), sound.getYPosF(),
										sound.getZPosF(), sound.getAttenuationType().getTypeInt(), f1);
							}
