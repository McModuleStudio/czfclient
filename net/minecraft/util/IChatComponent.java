			...
			if (p_deserialize_1_.isJsonPrimitive()) {
				...
				if (p_deserialize_1_.isJsonArray()) {
					...
					for (JsonElement jsonelement : jsonarray1) {
						IChatComponent ichatcomponent2 = this.deserialize(jsonelement, jsonelement.getClass(),
								p_deserialize_3_);
				...
				if (jsonobject.has("text")) {
					...
					ichatcomponent = new ChatComponentScore(JsonUtils.getString(jsonobject1, "name"),
							JsonUtils.getString(jsonobject1, "objective"));
				...
				if (jsonobject.has("extra")) {
					...
					for (int j = 0; j < jsonarray2.size(); ++j) {
						ichatcomponent
								.appendSibling(this.deserialize(jsonarray2.get(j), p_deserialize_2_, p_deserialize_3_));
					}
				}
			...
			if (p_serialize_1_ instanceof ChatComponentText && p_serialize_1_.getChatStyle().isEmpty()
				...
				if (!p_serialize_1_.getSiblings().isEmpty()) {
					...
					for (IChatComponent ichatcomponent : p_serialize_1_.getSiblings()) {
						jsonarray.add(this.serialize((IChatComponent) ichatcomponent, ichatcomponent.getClass(),
								p_serialize_3_));
					}
					...
					if (chatcomponenttranslation.getFormatArgs() != null
						...
						for (Object object : chatcomponenttranslation.getFormatArgs()) {
							if (object instanceof IChatComponent) {
								jsonarray1.add(this.serialize((IChatComponent) ((IChatComponent) object),
										object.getClass(), p_serialize_3_));
							} else {
