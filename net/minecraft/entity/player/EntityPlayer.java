...
public abstract class EntityPlayer extends EntityLivingBase {
	...
	public EntityPlayer.EnumStatus trySleep(BlockPos bedLocation) {
		if (!this.worldObj.isRemote) {
			...
			if (Math.abs(this.posX - (double) bedLocation.getX()) > 3.0D
					|| Math.abs(this.posY - (double) bedLocation.getY()) > 2.0D
				...
				if (itemStackIn != null && itemStackIn.getItem() != null) {
					if (itemStackIn.getItem() instanceof ItemArmor) {
						if (EntityLiving.getArmorPosition(itemStackIn) != k) {
							...
							|| itemStackIn.getItem() != Items.skull && !(itemStackIn.getItem() instanceof ItemBlock)) {
						return false;
					}
				}
