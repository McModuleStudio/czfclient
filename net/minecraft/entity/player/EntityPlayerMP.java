...
public class EntityPlayerMP extends EntityPlayer implements ICrafting {
				...
				if (chunkcoordintpair != null) {
					if (this.worldObj.isBlockLoaded(
						...
						if (chunk.isPopulated()) {
							list.add(chunk);
							list1.addAll(((WorldServer) this.worldObj).getTileEntitiesIn(
			...
			if (this.getHealth() + this.getAbsorptionAmount() != this.combinedHealth) {
				...
				for (ScoreObjective scoreobjective : this.getWorldScoreboard()
					...
					this.getWorldScoreboard().getValueFromObjective(this.getName(), scoreobjective)
							.func_96651_a(Arrays.<EntityPlayer>asList(new EntityPlayer[] { this }));
				}
			}
	...
	/**
	 * Updates all biomes that have been explored by this player and triggers
	 * Adventuring Time if player qualifies.
	 */
	...
	public void displayGui(IInteractionObject guiOwner) {
		...
		this.playerNetServerHandler.sendPacket(
				new S2DPacketOpenWindow(this.currentWindowId, guiOwner.getGuiID(), guiOwner.getDisplayName()));
	...
	public void displayGUIChest(IInventory chestInventory) {
		...
		if (chestInventory instanceof IInteractionObject) {
			this.playerNetServerHandler.sendPacket(
					new S2DPacketOpenWindow(this.currentWindowId, ((IInteractionObject) chestInventory).getGuiID(),
	...
	public void displayVillagerTradeGui(IMerchant villager) {
		...
		this.playerNetServerHandler.sendPacket(new S2DPacketOpenWindow(this.currentWindowId, "minecraft:villager",
				ichatcomponent, iinventory.getSizeInventory()));
	...
	public void displayGUIHorse(EntityHorse horse, IInventory horseInventory) {
		...
		this.playerNetServerHandler.sendPacket(new S2DPacketOpenWindow(this.currentWindowId, "EntityHorse",
				horseInventory.getDisplayName(), horseInventory.getSizeInventory(), horse.getEntityId()));
	...
	public void sendProgressBarUpdate(Container containerIn, int varToUpdate, int newValue) {
		this.playerNetServerHandler
				.sendPacket(new S31PacketWindowProperty(containerIn.windowId, varToUpdate, newValue));
	}

	public void func_175173_a(Container p_175173_1_, IInventory p_175173_2_) {
		for (int i = 0; i < p_175173_2_.getFieldCount(); ++i) {
			this.playerNetServerHandler
					.sendPacket(new S31PacketWindowProperty(p_175173_1_.windowId, i, p_175173_2_.getField(i)));
		}
	}
	...
	public void addStat(StatBase stat, int amount) {
		if (stat != null) {
			...
			for (ScoreObjective scoreobjective : this.getWorldScoreboard()
					.getObjectivesFromCriteria(stat.func_150952_k())) {
	...
	public void func_175145_a(StatBase p_175145_1_) {
		if (p_175145_1_ != null) {
			...
			for (ScoreObjective scoreobjective : this.getWorldScoreboard()
					.getObjectivesFromCriteria(p_175145_1_.func_150952_k())) {
	...
	public boolean canCommandSenderUseCommand(int permLevel, String commandName) {
		if ("seed".equals(commandName) && !this.mcServer.isDedicatedServer()) {
			...
			if (this.mcServer.getConfigurationManager().canSendCommands(this.getGameProfile())) {
				UserListOpsEntry userlistopsentry = (UserListOpsEntry) this.mcServer.getConfigurationManager()
