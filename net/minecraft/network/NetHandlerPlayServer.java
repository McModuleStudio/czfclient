...
public class NetHandlerPlayServer implements INetHandlerPlayServer, ITickable {
	...
	public void processInput(C0CPacketInput packetIn) {
		...
		this.playerEntity.setEntityActionState(packetIn.getStrafeSpeed(), packetIn.getForwardSpeed(),
				packetIn.isJumping(), packetIn.isSneaking());
	}
		...
		if (packetIn.getPlacedBlockDirection() == 255) {
			...
			if (this.hasMoved
				...
				this.playerEntity.theItemInWorldManager.activateBlockOrUseItem(this.playerEntity, worldserver,
						itemstack, blockpos, enumfacing, packetIn.getPlacedBlockOffsetX(),
	...
	public void processClickWindow(C0EPacketClickWindow packetIn) {
		...
		if (this.playerEntity.openContainer.windowId == packetIn.getWindowId()
			...
			if (this.playerEntity.isSpectator()) {
				...
				ItemStack itemstack = this.playerEntity.openContainer.slotClick(packetIn.getSlotId(),
						packetIn.getUsedButton(), packetIn.getMode(), this.playerEntity);
	...
	public void processTabComplete(C14PacketTabComplete packetIn) {
		...
		for (String s : this.serverController.getTabCompletions(this.playerEntity, packetIn.getMessage(),
				packetIn.getTargetBlock())) {
			list.add(s);
		}

		this.playerEntity.playerNetServerHandler
				.sendPacket(new S3APacketTabComplete((String[]) list.toArray(new String[list.size()])));
	}
